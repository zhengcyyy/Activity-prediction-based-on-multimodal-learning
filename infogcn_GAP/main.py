...
from loss import LabelSmoothingCrossEntropy, get_mmd_loss, KLLoss
from model.infogcn import InfoGCN, TextCLIP
import torch.nn as nn
from utils import get_vector_property
from utils import BalancedSampler as BS
from utils import gen_label, create_logits
from Text_Prompt import *
from torch.cuda.amp import GradScaler, autocast
# 注意学习率问题 ntu120为0.1，ntu60为0.01
# python main.py --half=True --batch_size=96 --test_batch_size=96 \
#     --step 90 100 --num_epoch=110 --n_heads=3 --num_worker=4 --k=8 \
...
# GAP
classes, num_text_aug, text_dict = text_prompt_openai_pasta_pool_4part() # old_text
#classes, num_text_aug, text_dict = text_seg_4part(prediction_ratio)  # new_text
            ...
            with autocast(enabled=self.arg.half):
                ...
                for ind in range(num_text_aug):
                    ...
                    if ind == 0:
                        ...
                        ground_truth = torch.tensor(label_g,dtype=feature_dict[self.arg.model_args['head'][0]].dtype).cuda()
                    else:
                        logits_per_image, logits_per_text = create_logits(part_feature_list[ind-1],text_embedding,logit_scale[:,ind].mean())

                        ground_truth = torch.tensor(label_g,dtype=part_feature_list[ind-1].dtype).cuda()
                ...
                loss = self.arg.lambda_2* mmd_loss + self.arg.lambda_1* l2_z_mean + cls_loss + 0.8 * sum(loss_te_list) / len(loss_te_list)
            # backward
...
def main():
    ...
    arg.work_dir = f"results_0.8_seg1_newtext/{arg.dataset}_{arg.datacase}_{arg.k}_{prediction_ratio}"
    #rg.weights = f"/data/data1/zhengchaoyang/infogcn_GAP/results_0.8_seg1_oldtext/ntu_NTU60_CS_8_{prediction_ratio}/runs-70.pt"
