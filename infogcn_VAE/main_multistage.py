...
# python main_multistage.py --half=True --batch_size=96 --test_batch_size=96 \
...
print('prediction_ratio:', prediction_ratio)
...
class Processor():
    ...
    def train_stage1(self, epoch, save_model=False):
        self.model.train()
    ...
    def train_stage2(self, epoch, save_model=False):
        self.model.train()
        self.print_log('Training epoch: {}'.format(epoch + 1))
        self.adjust_learning_rate(epoch)
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if epoch == 0:
                    self.print_log('Training stage 2')
                if epoch < 0:
                    self.train_stage1(epoch, save_model=save_model)
                else:
                    self.train_stage2(epoch, save_model=save_model)
